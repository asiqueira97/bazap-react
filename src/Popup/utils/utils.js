...
export function groupByClient(messages) {
  ...
  export function getMessagesOrdered(messages, times) {
    ...
    return filterByTime ? sortMessages(sortMessagesByTime(messages,times.startTime,times.endTime)) : sortMessages(messages)
  }
  ...
  export function mountProductListByPerson(data) {
    ...
    localStorage.setItem('products-by-client', JSON.stringify(grouped))
  }
...
export const optionsProductsPage = [
  ...
  {name:'publicados', active: false},
  {name:'disponíveis', active: false},
]
